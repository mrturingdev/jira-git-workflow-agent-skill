...
- **With an argument** (e.g. `/checkout "my current in-progress ticket about topup"`):
  ...
  2. Show which branch is being checked out **from** and **to** (e.g. `from main → to TOPUP-123-topup-flow`).
  3. Create/checkout the branch from the fix version of the ticket, naming it from the ticket key + short slug.
